...
- `[~]` Phase 3 Clojure semantic-core: namespace/var identity, test linkage, caller/callee resolution, multimethod dispatch, and macro-aware ownership now cover syntax-quoted, composed, and local-helper-generated forms substantially better, but the tail is still not compiler-grade
- `[~]` Phase 3 Elixir semantic-core: alias/import handling, `use`-aware unqualified-call expansion, arity-sensitive targeting, `defdelegate` linkage, and ExUnit test linkage now exist, but deeper ownership/disambiguation work still remains
