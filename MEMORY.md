...
- **`plans/018` Stage 7 part one is delivered (2026-09-08); removal is scheduled, not taken.** **Deprecated: `:java_engine` and `:typescript_engine`** — the provider plan admits tiers by observed status and authority, so naming an engine can only agree with the plan or be ignored, and an option that cannot change the outcome is a misleading control. **Not deprecated**: `:clojure_engine` and `:elixir_engine` (lanes outside the migration, where the option still chooses) and `:tree_sitter_enabled` (still governs whether the structural tier exists at all). Signal is machine-readable and rides the existing key: a build passing one gets `provider_summary.deprecated_options` back, deliberately not a log line. **Window: one week from 2026-09-08; removal proposed after 2026-09-15 as its own decision (owner).** Migration path and rollback are both `:provider_pipeline "off"`. **Expansion decision: no further language migrates now** — the bar is evidence, not symmetry, `plans/020` is still paused so no comparative measurement exists for any lane, and what Stage 6 measured says the mechanism works rather than that another lane would benefit. A candidate needs a reproducibly runnable semantic provider, a corpus where the exact tier disagrees with the lexical one often enough to matter, and a measured retrieval difference; Python and Elixir lack the first. **Stale metric found and fixed while writing the schedule**: `files_degraded` still counted `parser_mode "fallback"` units, which bugs/005 had made impossible, so it had been silently zero for every build; it reads the file diagnostic now and a test asserts it agrees with that diagnostic's count.
